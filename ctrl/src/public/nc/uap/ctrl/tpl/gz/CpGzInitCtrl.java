...
public class CpGzInitCtrl implements WindowController, Serializable {
	...
	public void onCondition(MouseEvent mouseEvent) {
		...
		String pktemplate = (String) row.getValue(ds
				.nameToIndex("pk_gz_template"));
		ctx.getWindowContext().addAppAttribute("metaclass", metaclass);
		ctx.getWindowContext().addAppAttribute("pk_gz_template", pktemplate);
	...
	public void saveBtClick(MouseEvent mouseEvent) {
		...
		ICpGzTemplateInnerService service = NCLocator.getInstance().lookup(ICpGzTemplateInnerService.class);
		try {
			service.initTemplate((CpGzTemplateVO) vos[0]);
	...
	// String key = ds.getReqParameters().getParameterValue(
	...
	// int loadFieldIndex = ds.nameToIndex("loadfield");
